...
vec3 calcNormal(vec3 p) {
    float eps = 0.001;
    vec3 col;
    float dx = sceneSDF(p + vec3(eps,0,0), col) - sceneSDF(p - vec3(eps,0,0), col);
    float dy = sceneSDF(p + vec3(0,eps,0), col) - sceneSDF(p - vec3(0,eps,0), col);
    float dz = sceneSDF(p + vec3(0,0,eps), col) - sceneSDF(p - vec3(0,0,eps), col);
...
void main() {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        if (dist < 0.002) { hit = true; hitPos = pos; break; }
        if (t > maxDist) break;
        t += max(0.02, dist * 0.8);
    }
    ...
    if (hit) {
        ...
        vec3 diffuse = 0.8 * diff * col;
        vec3 specular = 0.3 * spec * vec3(1.0);
        color = ambient + diffuse + specular;
    } else {
        float glow = exp(-length(p) * 2.5);
        color = vec3(0.02, 0.0, 0.0) + glow * vec3(0.3, 0.0, 0.0);
    }
